...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, unique_test_env, default_hparams, hparams):
    args_list = []
    for trial_seed in range(n_trials):
        for dataset in dataset_names:
            for algorithm in algorithms:
                if unique_test_env >= 0:
                    all_test_envs = [[unique_test_env]]
                ...
                for test_envs in all_test_envs:
                    if default_hparams:
                        ...
                        hparam_seeds = range(n_hparams_from, n_hparams)
                    for hparams_seed in hparam_seeds:
                        train_args = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unique_test_env', type=int, default=-1)
    parser.add_argument('--default_hparams', action='store_true')
    ...
    args_list = make_args_list(
        ...
        unique_test_env=args.unique_test_env,
        default_hparams=args.default_hparams,
